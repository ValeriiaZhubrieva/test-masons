...
function menuInit() {
  document.addEventListener("click", function(e) {
    ...
    } else if (!e.target.closest("[data-fls-menu]") && !e.target.closest(".menu__body") && !e.target.closest("[data-fls-popup]")) {
      bodyUnlock();
    ...
    if (e.target.closest("[data-fls-user-menu]")) {
      document.documentElement.toggleAttribute("data-fls-user-menu-open");
    } else if (!e.target.closest("[data-fls-user-menu]") && !e.target.closest(".user-menu") && !e.target.closest("[data-fls-popup]")) {
      document.documentElement.removeAttribute("data-fls-user-menu-open");
    } else if (e.target.closest(".user-menu__link")) {
      document.documentElement.removeAttribute("data-fls-user-menu-open");
    }
  });
}
...
dropdownElements.forEach(function(dropDownWrapper) {
  ...
  const checkDropdownPosition = () => {
    ...
    if (spaceBelow < 0 && spaceAbove > Math.abs(spaceBelow)) {
      dropDownWrapper.classList.add("dropdown-top");
    } else {
      dropDownWrapper.classList.remove("dropdown-top");
    }
  };
  const openDropdown = () => {
    if (closeTimeout) {
  ...
  if (!isTouch && typeof isMobile !== "undefined" && !isMobile.any()) {
    dropDownWrapper.addEventListener("mouseenter", () => {
      openDropdown();
    });
    dropDownWrapper.addEventListener("mouseleave", (event) => {
      if (!dropDownWrapper.contains(event.relatedTarget)) {
        closeDropdown();
      }
    });
  }
  ...
  if (e.key === "Tab") {
    setTimeout(() => {
      dropdownElements.forEach((dropdown) => {
        const list = dropdown.querySelector("[data-dropdown-list]");
        const btn = dropdown.querySelector("[data-dropdown-button]");
        if (list?.classList.contains("visible") && !dropdown.contains(document.activeElement)) {
          list.classList.remove("visible");
          btn?.classList.remove("active");
          btn?.setAttribute("aria-expanded", "false");
          setTimeout(() => {
...
window.initSelectBlocks = function() {
  const selectBlocks = document.querySelectorAll("[data-selblock]");
  if (!selectBlocks.length) return;
  selectBlocks.forEach((select) => {
    const header = select.querySelector("[data-sel-header]");
    const body = select.querySelector("[data-sel-body]");
    const title = select.querySelector("[data-sel-title]");
    const searchInput = select.querySelector("[data-sel-search]");
    const closeBtn = select.querySelector("[data-sel-close]");
    const hiddenInput = select.querySelector("[data-sel-input]");
    const items = select.querySelectorAll("[data-sel-item]");
    const groups = select.querySelectorAll("[data-sel-group]");
    updatePlaceholder(select);
    header?.addEventListener("click", (e) => {
      e.stopPropagation();
      document.querySelectorAll("[data-selblock]._active").forEach((other) => {
    ...
    items.forEach((item) => {
      item.addEventListener("click", (e) => {
        e.stopPropagation();
        items.forEach((i) => i.classList.remove("ocf-selected"));
        item.classList.add("ocf-selected");
        if (title) {
          title.innerHTML = "";
          const box = item.querySelector("[data-sel-box]");
          const icon = item.querySelector("[data-sel-icon]");
          const code = item.querySelector("[data-sel-code]");
          const label = item.querySelector("[data-sel-label]");
          if (box) {
            title.appendChild(box.cloneNode(true));
          } else if (icon || code || label) {
            const wrap = document.createElement("span");
            icon && wrap.appendChild(icon.cloneNode(true));
            code && wrap.appendChild(code.cloneNode(true));
            label && wrap.appendChild(label.cloneNode(true));
    ...
    searchInput?.addEventListener("input", function() {
      ...
      items.forEach((item) => {
        const code = item.querySelector("[data-sel-code]")?.textContent || "";
        const label = item.querySelector("[data-sel-label]")?.textContent || "";
        const fallback = item.textContent || "";
        const text = `${code} ${label} ${fallback}`.toLowerCase();
        item.classList.toggle("_hidden", !text.includes(value));
      });
      groups.forEach((group) => {
        const groupItems = group.querySelectorAll("[data-sel-item]");
        const groupTitle = group.querySelector("[data-sel-group-title]");
        const hasVisible = [...groupItems].some((i) => !i.classList.contains("_hidden"));
        groupTitle?.classList.toggle("_hidden", !hasVisible);
      });
    });
  });
  document.addEventListener("click", (e) => {
    document.querySelectorAll("[data-selblock]._active").forEach((sel) => {
  ...
  function closeSelect(select) {
    ...
    const search = select.querySelector("[data-sel-search]");
    if (search) search.value = "";
    select.querySelectorAll("[data-sel-item]._hidden").forEach((i) => i.classList.remove("_hidden"));
    select.querySelectorAll("[data-sel-group-title]._hidden").forEach((t) => t.classList.remove("_hidden"));
  }
  function updatePlaceholder(select) {
    const hasSelected = select.querySelector("[data-sel-item].ocf-selected");
    select.classList.toggle("placeholder", !hasSelected);
  }
};
...
class CircularTimer {
  constructor(container) {
    ...
    this.duration = parseInt(container.dataset.duration) || 60;
    this.timeLeft = this.duration;
    this.progressCircle = container.querySelector(".stopwatch__progress");
    this.textElement = container.querySelector(".stopwatch__text");
    this.radius = 90;
  ...
  updateDisplay() {
    const displayTime = this.timeLeft < 10 ? "0" + this.timeLeft : this.timeLeft;
    this.textElement.textContent = displayTime;
...
const timerContainers = document.querySelectorAll(".stopwatch");
if (timerContainers.length) {
  timerContainers.forEach((timerContainer) => {
    new CircularTimer(timerContainer);
  });
}
...
initInputEffects();
document.addEventListener("DOMContentLoaded", () => {
  const formContainer = document.querySelector(".form-exchange");
  if (formContainer) {
    formContainer.addEventListener("click", (e) => {
      const field = e.target.closest(".form-exchange__field");
      if (field) {
        const input = field.querySelector("input, textarea, select");
        if (input && !e.target.matches("input, textarea, select, a, button")) {
          input.focus();
        }
      }
    });
...
if (timerEls.length) {
  timerEls.forEach((timerEl) => startTimer(timerEl));
}
function startTimer(timerEl) {
  const raw = timerEl.textContent.replace(/[^\d]/g, "");
  const minutes = Number(raw);
  if (!minutes || minutes <= 0) {
    console.warn("data-timer не содержит корректные минуты", timerEl);
    return;
  }
  ...
  const tick = () => {
    if (total <= 0) {
      ...
      timerEl.dispatchEvent(new CustomEvent("timerEnd"));
      return;
    }
    total--;
    timerEl.textContent = formatTime(total);
    timeoutId = setTimeout(tick, 1e3);
    timers.set(timerEl, timeoutId);
  };
  tick();
}
...
document.addEventListener("DOMContentLoaded", () => {
  const fileInputs = document.querySelectorAll(".file-block__input");
  if (fileInputs.length) {
    fileInputs.forEach((input) => {
      const fileBlock = input.closest(".file-block");
      const textElement = fileBlock.querySelector(".file-block__text");
      const defaultText = textElement.textContent.trim();
      const maxSizeMB = Number(fileBlock.dataset.maxSize) || 9;
      ...
      input.addEventListener("change", (e) => {
        ...
        if (files && files.length > 0) {
          ...
          if (file.size > MAX_FILE_SIZE) {
            textElement.textContent = `${fileBlock.dataset.errorText || "Файл слишком большой"} (макс. ${maxSizeMB} МБ)`;
            fileBlock.classList.add("is-error");
            fileBlock.classList.remove("has-file");
            input.value = "";
            return;
          }
          textElement.textContent = file.name;
          fileBlock.classList.add("has-file");
          fileBlock.classList.remove("is-error");
        } else {
